...
- **残差插值**: 在 Oklab 空间学习 `输出色 - 输入色` 的调色位移，减少直接平均目标色造成的风格损失
- **可靠度加权**: 对局部输入色中调色位移不一致的样本降低权重，而不是直接删除强风格映射
- **边缘保留平滑**: 只平滑 LUT 残差，并按残差差异降低跨风格边界的混色
- **鲁棒单调拟合**: 使用 L1 Isotonic Regression 将主通道轴拟合为单调序列，以最小绝对改动保留原 LUT，并降低异常值的影响
